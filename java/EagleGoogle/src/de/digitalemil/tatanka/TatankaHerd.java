...
public class TatankaHerd extends ThingContainer {
		...
		for (int i = 0; i < n; i++) {
			...
			//	things[i].rotateRoot(-20 + Part.getRandom(0, 40));
				...
				if (things[i].getCollisionHandler().collisionHappend() != CollisionHandlerImpl.NOCOLLISION) {
					System.out.println("Tatanka collide: "+i+ " "+rx+" "+ry+" "+hw+" "+hh);
					things[i].rollbackTX();
	...
	private int getRotation(float speedx, float speedy, float lakotaX) {
		...
		if(tatanka.didCollide() || tatanka.getRotation()< -10 || (tatanka.getRotation()> 10 && tatanka.getRotation()< 350))
			return 0;
	...
	public void update(float speedx, float speedy, float lakotaX, float lakotaY) {
		...
		for (int i = 0; i < n; i++) {
			...
			sin = Part.mysin[phi];
			cos = -Part.mycos[phi];
			sx = -cos * tspeed  * Globals.getScale() *3;
			sy = -sin * tspeed * Globals.getScale() *3;
	/*		if (things[alpha].getY() + things[alpha].getRy() < -2
					* Globals.getH2())
				sx = tspeed + speedy;
			if (things[alpha].getY() + things[alpha].getRy() > 2 * Globals
					.getH2())
				sy = tspeed - speedy;*/
			if (faster) {
