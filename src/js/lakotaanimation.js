...
LakotaAnimation.prototype.shoot = function(shootAtX, shootAtY) {
	...
	this.shooting = true;
	var x = shootAtX - w / 2 - this.lakota.bow.getCoordinateTap().getX();
	var y = h / 2 - shootAtY + this.lakota.bow.getCoordinateTap().getY();
...
LakotaAnimation.prototype.increaseLevelImpl = function() {
	...
	case 2:
		break;
	case 3:
		var l = Math.sqrt(canvaswidth * canvaswidth + canvasheight
				* canvasheight)
				* scale;
		...
		var phi = (this.arrow.rot + this.arrow.rrot);
		phi = Math.round(phi);
		while (phi < 0)
			phi += 360;
		phi %= 360;
