...
TatankaModell.prototype.setup= function() {
	...
	allThings[pos]= this.texts;
	allThings[pos].translate(-w2+64*scale, h2, 0);
	this.texts.setArrows(this.lakotas[this.activelakota].arrows);
	pos++;

	allThings[pos]= new ImageThing("res/arrowsetalmin.png", (123* scale/2), (354* scale/2));
	allThings[pos].translate(-w2+(123* scale/2)/2, h2-(374* scale/2)/2, 0 );
	pos++;
	numberOfThings = pos;
	this.joystick.reset();
};
...
TatankaModell.prototype.touchStart= function(x, y) {
	console.log("shoot1: "+this.texts);

	if(!this.lakotas[this.activelakota].shoot(x, y)) {
		console.log("shoot2: "+this.texts);

		this.joystick.down(x, y);
	}
	else {
		console.log("shoot: "+this.texts);
